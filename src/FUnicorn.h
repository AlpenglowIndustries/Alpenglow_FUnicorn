...
/************************************************
  ...
  should be portable to most boards running an ATMega328P.
...
// necessary if the button is used to interrupt the processor (or wake it from
// sleep)
#define DEBOUNCE 30
...
#define LOW_PWR_BUTT \
    1  // external button only with sleep mode, good for battery power
...
class FUnicorn {
   public:
    ...
    // check these out in FUnicorn.cpp and use them as examples to build your
    // own! Usage ex: fun.hornBlink();
    void hornBlink(void);
    void blinkDemo(void);
   ...
   private:
    ...
    void initFuckHornTimer(
        void);  // sets up timer module used for Fuck and Horn LEDs
    void initBangYouTimer(
        void);  // sets up timer module used for Bang and You LEDs
};
