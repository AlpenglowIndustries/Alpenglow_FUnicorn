...
// signed value would be int8_t, = -128 to 127.  Asymmetric because 0 is a
// value.
...
uint8_t checkButt() {
    ...
    if (buttJustPressed && (millis() - buttTime > DEBOUNCE)) {
        ...
        EIMSK |=
            (1 << INT0);  // enables INT0, same shorthand as above except that
                          // "OR" "|" sets a bit to 1 somewhat equivalent to
                          // attachInterrupt(), re-enables it without changing
                          // behavior (already set to falling edge)
