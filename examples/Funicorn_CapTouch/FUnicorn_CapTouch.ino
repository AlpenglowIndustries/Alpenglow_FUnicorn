...
#define CAP_BUTT_PRESS \
    20  // lower value is more senstive to touch, default = 50 \
...
void loop() {
    ...
    int touchValue =
        ADCTouch.read(A0);  // no second parameter defaults to 100 samples
    touchValue -= capRef;   // removes offset

    if (touchValue >
        CAP_BUTT_PRESS) {  // determines that the unicorn has been touched
        ...
        counter++;
        delay(10);  // slows down the loop a little, doesn't check for touches
                    // as often
    }
}
