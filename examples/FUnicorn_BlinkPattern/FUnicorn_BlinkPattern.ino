...
uint8_t checkButt() {
  ...
  if (buttJustPressed && (millis() - buttTime > DEBOUNCE)) {
    ...
    EIMSK |= (1 << INT0);       // enables INT0, same shorthand as above except that "OR" "|" sets a bit to 1
                                // somewhat equivalent to attachInterrupt(), re-enables it without changing behavior
                                //  (already set to falling edge in FUnicorn.cpp initButt(), and interrupt routine is ISR(INT0_vect))
